...
- **소프트웨어 아키텍쳐** ★
  ...
  - 소프트웨어 **구성요소**와 구성요소들간의 **관계**를 나타낸 **시스템 구조**
...
- **웹 애플리케이션 서버(WAS) 탑재**
- **서버 프로그램**은 웹 서버로부터 받은 요청을 처리하여 **반환**하는 역할
- 언어는 Java, Python, JS, PHP, Ruby 등

#### (2) 프레임워크 정의★

> 해당 언어로 **서버프로그램 개발**하도록 지원하는 뼈대, 골대

#### (3) 유형

- **맵 리듀스** ★
  - 구글에서 **대용량 데이터** 처리를 **분산 병렬 컴퓨팅**에서 처리하기 위해 제작
...
- **내공외제스자**
  ...
  - 내용 (**직접** 참조 수정)
    ...
    모듈 간의 데이터, **지역변수 직접** 참조
  
  - 공통 (공유되는 공통 데이터 영역 / 파라미터가 아닌 **모듈 밖에 선언된 전역 변수** 참조하고 전역 변수 갱신하는 상호작용)
  
  - 외부 (두개의 모듈이 **외부 다른 모듈** 참조 / 데이터 포맷, 통신프로토콜, 또는 디바이스 인터페이스)
  
  - 제어 (다른 모듈 **흐름 제어**)
  
  - 스탬프 (두 모듈 **동일한 자료** 구조)
  
    모듈 간의 인터페이스로 **배열이나 객체** 등 **자료구조 전달**
